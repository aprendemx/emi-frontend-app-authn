...
const RegistrationPage = (props) => {
  ...
  const registerUser = () => {
    ...
    if (formFields.estado && formFields.estado.estadoCode) {
      // Estado se transformará en el payload final
    } else {
    ...
    // Transformar estado al formato esperado por el backend
    if (formFields.estado && formFields.estado.estadoCode) {
      _payload.estado = formFields.estado.estadoCode;
    }

    // Transformaciones finales del payload ANTES del validador
    if (isLlaveMX) {
      // Para LLAVE MX: enviar campos vacíos porque NO vienen de la API
      _payload.ocupacion = '';
    ...
    // En vez de machacar el payload con formFields,
    // usamos el payload CORRECTO ya normalizado
    let payload = { ..._payload };

    // Si hay provider social, eliminar password
    if (currentProvider) {
